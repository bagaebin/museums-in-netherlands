...
- `Locker`: 문(door)·내부 배경(detail background)·콘텐츠(detail content)을 `open`/`closed` 상태로 구동하며, 문 여닫이는 클릭으로만 트리거된다. 열린 상태에서 650ms 이상 호버하면 "held"로 확장되어 배경이 스테이지 전체를 채우고 팝업을 띄운다.
