...
### 현재 코드 기준으로 확인된 실제 동작
- 열린 상태에서 호버를 하지 않아도 상세 콘텐츠가 바로 나타난다. 즉, "클릭→열림"만으로 팝업이 표시되어 위 호버 지연 조건을 만족하지 않는다.
- 드래그 중에는 hover 타이머가 취소되지만, 빠르게 드래그할 때 위치가 순간적으로 튀거나 요소가 사라지는 현상이 보고되었다. 이는 드래그와 위치 애니메이션 제어가 충돌하는 상태로 추정된다.
