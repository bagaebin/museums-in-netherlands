...
import { motion, Variants, animate, useMotionValue } from 'framer-motion';
...
export function Locker({
  ...
  useEffect(() => {
    ...
    const xControls = animate(x, position.x, { type: 'spring', stiffness: 500, damping: 45 });
    const yControls = animate(y, position.y, { type: 'spring', stiffness: 500, damping: 45 });
  ...
  const isDetailVisible = isHeld;
  const radius = isDetailVisible ? expansionRadius : 700;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        variants={variants}
        animate={isDetailVisible ? 'open' : 'closed'}
        initial="closed"
        custom={radius}
        aria-hidden
      />
      ...
      <motion.div
        ...
        animate={isDetailVisible ? 'open' : 'closed'}
        initial="closed"
        style={{ pointerEvents: isDetailVisible ? 'auto' : 'none' }}
      >
        <h4>{museum.name}</h4>
