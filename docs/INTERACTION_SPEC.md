...
## 홀드-투-익스팬드 배경
- 포인터가 사물함 위에 머무른 채 650ms 이상 유지되면 "held" 상태로 전환된다.
- held 상태에서는 배경 clip-path가 스테이지 대각선 길이와 동일한 반경으로 커지며, 사물함 경계를 넘어 스테이지 전체를 채운다.
- held 상태가 해제되면(포인터 이탈, 드래그 시작 등) 배경과 콘텐츠가 다시 `closed`로 되돌아간다.
